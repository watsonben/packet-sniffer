...
icmp_header =   ('01 02'        #Type and code
        'ZZ ZZ'
#        '00 00 00 00'
        '00 00 00 00')                #Checksum - calculated and replaced later
...
def generatePCAP(message,port,pcapfile):
    icmp_len = getByteLength(message) + getByteLength(icmp_header)
    icmp = icmp_header
    icmp = icmp.replace('ZZ ZZ', "%04x"%42519)#ip_checksum(icmp.replace('ZZ ZZ', '00 00')))
    ip_len = icmp_len + getByteLength(ip_header)
    ...
    bytestring = pcap_global_header + pcaph + eth_header + ip + icmp + message
